...
if (length(unique(selected_vars_df$tool_reliability[selected_vars_df$tool_reliability != "none" &
  ...
  sapply(names(test_retest_reliability_tools_stats_w2_w5), function(x){
    nn <- x
    writexl::write_xlsx(list(r12 = data.frame(r12 = unname(test_retest_reliability_tools_stats_w2_w5[[nn]][["r12"]])),
                             alpha = tibble::rownames_to_column(as.data.frame(test_retest_reliability_tools_stats_w2_w5[[nn]][["alpha"]])),
                             item.stats = tibble::rownames_to_column(test_retest_reliability_tools_stats_w2_w5[[nn]][["item.stats"]]),
                             multilevel_component = tibble::rownames_to_column(test_retest_reliability_tools_stats_w2_w5[[nn]][["ml"]][["components"]]),
                             multilevel_reliability = data.frame(R1F = unname(test_retest_reliability_tools_stats_w2_w5[[nn]][["ml"]][["R1F"]]),
                                                                 RkF = unname(test_retest_reliability_tools_stats_w2_w5[[nn]][["ml"]][["RkF"]]),
                                                                 R1R = unname(test_retest_reliability_tools_stats_w2_w5[[nn]][["ml"]][["R1R"]]),
                                                                 RkR = unname(test_retest_reliability_tools_stats_w2_w5[[nn]][["ml"]][["RkR"]]),
                                                                 Rc = unname(test_retest_reliability_tools_stats_w2_w5[[nn]][["ml"]][["Rc"]])
                                                                 ),
                        ...
                        path = base::file.path(output_Dir, paste0("test_retest_reliability_w2_w5_",nn,".xlsx") )
                        )
